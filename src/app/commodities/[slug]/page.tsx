...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!meta) return { title: "Materia prima no encontrada" };
  return { title: `${meta.shortName} (${meta.unit}) | Materias Primas | Altius` };
}
...
export default async function CommodityDetailPage({
  ...
  const [detail, newsResult] = await Promise.all([
    getCommodityDetail(meta.symbol),
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-5 py-10 space-y-10">
      {/* Botón de Retorno y Cabecera de la Materia Prima */}
      <div className="space-y-4">
        ...
        >
          ...
          <span>Volver a todas las materias primas</span>
        </Link>

        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span className="font-mono text-amber-300 font-bold text-[14px] bg-carbon-surface px-3 py-1 rounded-full border border-gunmetal">
                {meta.shortName}
              </span>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-pure-white text-[22px] font-medium tracking-tight">
            {SHOW_TECHNICAL ? "Evolución de Precio y Análisis Técnico" : "Evolución del Precio"}
          </h2>
          <span className="text-muted-steel font-mono text-[12px]">
            {SHOW_TECHNICAL
              ? `${meta.unit} · Indicadores SMA 20/50/200 · Bollinger · RSI · MACD`
              : `${meta.unit} · Histórico oficial de cotización`}
          </span>
        </div>
      ...
      {/* Flujo de Noticias Relevantes de la Materia Prima */}
      <section className="space-y-4">
        <h2 className="font-display text-pure-white text-[22px] font-medium tracking-tight">
          Noticias y Pulso del Mercado
        </h2>
        <CompanyNewsFeed
