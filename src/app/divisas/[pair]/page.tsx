...
export async function generateMetadata({ params }: { params: Promise<{ pair: string }> }) {
  ...
  if (!meta) return { title: "Divisa no encontrada" };
  return { title: `${meta.shortName} (${meta.baseCurrency}/${meta.quoteCurrency}) | Divisas | Altius` };
}
...
export default async function CurrencyDetailPage({
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-5 py-10 space-y-10">
      {/* Botón de Retorno y Cabecera de la Divisa */}
      <div className="space-y-4">
        ...
        >
          ...
          <span>Volver a todas las divisas</span>
        </Link>
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          ...
          <DataSourceBadge
            ...
            detail={`Publicación estadística H.10 del Federal Reserve Board (${meta.fredSeriesId}).`}
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-pure-white text-[22px] font-medium tracking-tight">
            {SHOW_TECHNICAL ? "Evolución del Tipo de Cambio y Análisis Técnico" : "Evolución del Tipo de Cambio"}
          </h2>
          <span className="text-muted-steel font-mono text-[12px]">
            {SHOW_TECHNICAL
              ? `${meta.quoteCurrency} · SMA 20/50/200 · Bollinger · RSI · MACD`
              : `${meta.quoteCurrency} · Histórico oficial del tipo de cambio`}
          </span>
        </div>
      ...
      {/* Tabla de Estadísticas Históricas y Rendimientos */}
      <section className="bg-carbon-surface border-gunmetal rounded-2xl border p-6">
        <h3 className="font-display text-pure-white text-[18px] font-medium tracking-tight mb-5">
          Estadísticas Históricas y Rendimientos de Tipo de Cambio
        </h3>
        ...
        <div className="mt-6 pt-6 border-t border-gunmetal/60 grid gap-6 sm:grid-cols-2 lg:grid-cols-4 text-[13px]">
          <div>
            ...
            <span className="font-mono text-pure-white font-medium text-[16px]">
              {summary.ath.toLocaleString("es-ES", { maximumFractionDigits: 4 })}
            </span>
            <span className="text-muted-steel block text-[11px] mt-0.5">Registrado el {summary.athDate}</span>
          </div>
          ...
          <div>
            ...
            <span className="text-muted-steel block text-[11px] mt-0.5">Distancia al pico histórico</span>
          </div>

          <div>
            <span className="text-muted-steel block text-[11px] font-mono">MÁXIMO 52 SEMANAS</span>
            <span className="font-mono text-pure-white font-medium text-[16px]">
              {summary.high52w.toLocaleString("es-ES", { maximumFractionDigits: 4 })}
            </span>
            <span className="text-muted-steel block text-[11px] mt-0.5">Cota superior anual</span>
          </div>

          <div>
            <span className="text-muted-steel block text-[11px] font-mono">MÍNIMO 52 SEMANAS</span>
            <span className="font-mono text-pure-white font-medium text-[16px]">
              {summary.low52w.toLocaleString("es-ES", { maximumFractionDigits: 4 })}
            </span>
            <span className="text-muted-steel block text-[11px] mt-0.5">Soporte anual</span>
          </div>
        </div>
      </section>

      {/* Flujo de Noticias Relevantes y Política Monetaria */}
      <section className="space-y-4">
        <h2 className="font-display text-pure-white text-[22px] font-medium tracking-tight">
          Noticias y Política Monetaria
        </h2>
        <CompanyNewsFeed
